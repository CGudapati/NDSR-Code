...
// This reader is just okay. We can most certainly make it much better
...
std::vector<std::string> split_NDSR(const std::string &s, char delimiter)
{
...
NDSR_data read_instance_NDSR(const std::string filepath, double weight_multiplier)
{
        ...
        for (std::string line2; getline(infile2, line2);)
        {
                ...
                if (line2.find(arcs_start_str) == 0u)
                {
                        for (int i = 0; i < A; ++i)
                        {
                                ...
                                // std::cout << "(" << arc_tail << ", " << arc_head << ", " << arc_variable_cost << "),\n";
