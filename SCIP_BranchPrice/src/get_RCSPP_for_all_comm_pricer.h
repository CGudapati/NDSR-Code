#ifndef GET_RCSPP_FOR_ALL_COMMODITIES_pricer_H
#define GET_RCSPP_FOR_ALL_COMMODITIES_pricer_H
...
inline std::pair<std::vector<std::vector<int>>, std::vector<double>> get_rcspps_of_all_comms_in_pricer(
    NDSR_data& inst, RCSPP::RCSPP_graph& rcspp_graph, std::vector<std::vector<bool>>& nodes_prunned_flag_per_commodity, int num_commodities, int num_nodes, const std::vector<std::vector<double>>& comm_weight_info, const std::vector<std::pair<int, int>>& comm_s_t_info,
    std::vector<std::vector<double>>& w1_dist_matrix, std::vector<std::vector<double>>& w2_dist_matrix, const std::vector<std::vector<std::vector<double>>>& var_costs_per_all_k, std::vector<double> cutoff_rho_k)
{


        std::vector<std::vector<int>> rcspp_all_comms {};
        std::vector<double>           path_costs_list {};

        rcspp_all_comms.resize(num_commodities);
        path_costs_list.resize(num_commodities);
        ...
        for (int k = 0; k < num_commodities; ++k)
        {
                ...
                while (!node_stk.empty())
                {
                        ...
                        for (int i = 0; i < num_labels; ++i)
                        {
                                ...
                                for (auto& v : neighbors)
                                {
                                        ...
                                        {
                                                if (ll->m_cost + var_costs_per_all_k[k][u.m_node_idx][v] >= cutoff_rho_k[k])
                                                {
                                                        continue;
                                                }
                                                if (ll->m_flag == false)
                                                ...
                                                else if (dominated_pricer(k, u.m_node_idx, list_of_nodes[v], ll, var_costs_per_all_k, w1_adj, w2_adj))
                                                {
                                                ...
                                                {
                                                        to_be_expanded = true;
                                                        Label* lp      = new Label(v, ll, ll->m_cost + var_costs_per_all_k[k][u.m_node_idx][v], ll->m_m1_cons + w1_adj[u.m_node_idx][v], ll->m_m2_cons + w2_adj[u.m_node_idx][v], true);
                                                        list_of_nodes[v]->m_num_labels++;
                                                        list_of_nodes[v]->m_labels.push_back(lp);

                                                        if (v == terminal and ll->m_cost + var_costs_per_all_k[k][u.m_node_idx][v])
                                                        {
                                                                cutoff_rho_k[k] = ll->m_cost + var_costs_per_all_k[k][u.m_node_idx][v];
                                                        }

                                                        if (v != terminal and !node_stk_elems.has_elem(v))
                                                        {
                ...
                path_costs_list[k] = best_path_cst;
        ...
        return std::make_pair(rcspp_all_comms, path_costs_list);
}
#endif /* GET_RCSPP_FOR_ALL_COMMODITIES_pricer_H */
